...
class CVAEXperiment(pl.LightningModule):
    ...
    def on_validation_end(self) -> None:
        ...
        if self.trainer is not None and self.trainer.datamodule is not None:
            ...
            self.sample_images(test_dataloader)
        else:
...
#           print(f"Warning during random sampling: {e}")
    ...
    def sample_images(self, test_dataloader):
        # Get a batch of test images
        ...
        for idx in range(test_input.size(0)):  # Iterate over each input
            ...
            for z_idx, z in enumerate(selected_z_samples):
                ...
                # Decode the same `z` multiple times for verification
                repeated_images = [self.model.decode(z.unsqueeze(0)) for _ in range(5)]
                repeated_decoding_images.extend(repeated_images)
        ...
        repeated_decoding_images = torch.cat(repeated_decoding_images, dim=0)  # [total_repeated_images, channels, height, width]
        
        # Save coupled sampling results
        ...
        # Save repeated decoding results
        num_repeated = repeated_decoding_images.size(0)
        grid_size_repeated = int(num_repeated ** 0.5)
        vutils.save_image(
            repeated_decoding_images.data,
            os.path.join(self.logger.log_dir, "Samples", f"{self.logger.name}_RepeatedDecoding_Epoch_{self.current_epoch}.png"),
            normalize=True,
            nrow=grid_size_repeated
        )
