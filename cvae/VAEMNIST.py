# -*- coding: utf-8 -*-
"""
Created on Sat Dec  3 00:55:45 2022
...
from VAEModelMNIST import VAEModelMNIST as VAEModel
...
class VAE:
    ...
    def __init__(self, z_dim, n_filter_base=32, learning_rate=0.0005,
        ...
        self.model = VAEModel(z_dim, n_filter_base, seed, dtype, input_type)
    ...
    def compute_loss(self,
        ...
        x_recons_logits, z_sample, mean, logvar = self.model(
            x_true, input_type=self.input_type
            )
    ...
    def compute_loss_test(self, x_corrupt, x_true, loss_only=False, loss_coupling=0):
        ...
        if loss_coupling == 0:
          x_recons_logits, z_sample, mean, logvar = self.model(x_corrupt, input_type=self.input_type)
          loss, neg_ll_mean, kl_div_mean, logpx_z, kl_div = self.neg_elbo_loss(
