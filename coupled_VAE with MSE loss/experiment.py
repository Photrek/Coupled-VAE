...
class CVAEXperiment(pl.LightningModule):
    ...
    def sample_images(self):
            # Get a sample batch of 60 images from the test dataloader
            ...
            # Save original images only during epoch 0
            if self.current_epoch == 0:
                vutils.save_image(
                    test_input.data,
                    os.path.join(self.logger.log_dir, "Reconstructions", "original_images_epoch_0.png"),
                    normalize=True,
                    nrow=10  # Arrange images in a 6x10 grid
                )
                
            # Save reconstructions in a 6x10 grid
            vutils.save_image(
                recons.data,
            ...
            # Optional: Generate samples from the latent space
            try:
